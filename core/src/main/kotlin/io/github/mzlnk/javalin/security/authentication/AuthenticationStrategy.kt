 ...
 * per block: assigning [AuthenticationStrategy] again simply replaces the previous one (the same
 ...
 *
 * A strategy is either [Sync] (a blocking [Authenticator]) or [Async] (a non-blocking
 ...
 * made strategies via factory functions — `jwt { }`, `basicAuth { }` — that return a [Sync]
 * instance built from their own configuration block. Applications may also implement [Sync] or
 * [Async] directly for a fully custom authentication mechanism:
 *
 * ```kotlin
...
sealed interface AuthenticationStrategy {
    ...
    /**
     * A strategy backed by a blocking [Authenticator].
     *
    ...
    /**
     * A strategy backed by an opt-in, non-blocking [AsyncAuthenticator], for authentication that
     * performs remote I/O.
     *
