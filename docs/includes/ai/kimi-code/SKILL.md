...
description: Reference for adding authentication and authorization to a Javalin 7 application with the javalin-security plugin (io.github.mzlnk:javalin-security). Covers configuring `security.rules`, wiring the shipped strategies (Basic Auth, API Key, Opaque Token, Session, JWT), reading the caller in a handler, writing a custom `AuthenticationStrategy` or `Rule`, hardening JWT/WebSocket/CORS setup, and debugging 401/403 responses.
type: prompt
whenToUse: When the user writes, configures, or debugs code that imports from `io.github.mzlnk.javalin.security` — including adding an authentication strategy to a Javalin app, defining rules with `security.rules`, reading `ctx.identity` / `ctx.authentication`, implementing a custom `AuthenticationStrategy` or `Rule`, hardening JWT/WebSocket/CORS setup, or investigating why a route returns 401 or 403. Do NOT trigger for general Javalin questions that don't involve the security plugin.
---
