 ...
 *
 * This is opt-in behavior. To activate it, set `bearerChallenge = true` inside the `jwt { }` DSL
 * block, or pass an instance to `unauthorizedHandler(...)` on the builder:
 *
 * ```kotlin
 * http {
 *     jwt {
 *         decoder = myDecoder
 *         bearerChallenge = true        // emits WWW-Authenticate header
